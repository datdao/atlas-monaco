...
class CodeCompletion implements ICodeCompletion {
  ...
  getProvider(): monaco.languages.CompletionItemProvider {
    ...
    return {
      ...
      provideCompletionItems(
        ...
        const suggestions = listSuggestionByNestedScopes(
          hclParser.listNestedScopes()
        );
        ...
        if (
          ...
          if (
            nestedScopes.length > 0 &&
            !suggestionLvl0.find((v) => v.value == nestedScopes[0])
          ) {
            nestedScopes = [
              blocks[0]?.block,
              blocks[0]?.values[0],
          ...
          const referencedBlockValues =
            hclParser.findReferencedBlockValues(nestedScopes);
          return {
            suggestions: buildGlobalSearchItems(referencedBlockValues, range),
          };
        }
      ...
      switch (suggestion.type) {
        case SuggestionType.block:
          completionItems.push(
            this.buildBlockTemplate(suggestion.value, range, suggestion.config)
          );
          break;
        case SuggestionType.attribute:
          completionItems.push(
            this.buildAttributeTemplate(
              suggestion.value,
              range,
              suggestion.config
            )
        ...
        case SuggestionType.attributeValue:
          completionItems.push(
            this.buildValueTemplate(suggestion.value, range)
          );
          break;
      }
    });
  ...
  buildBlockTemplate(
    ...
    return {
      ...
      detail: "",
      insertText: addCursorIndexToQuestionMarks(
        key + " " + name + " {\n\t${?}\n}"
      ),
      insertTextRules:

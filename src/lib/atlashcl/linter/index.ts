...
class Linter {
  ...
  validateNonCompliantWord(
    ...
    const suggestions =
      this.hclNavigator.listSuggestionByNestedScopes(nestedScopes);
    ...
    const isTypeNonCompliantWord =
      suggestions.find((suggestion) => {
        return (
          suggestion.value.startsWith(word) ||
          suggestion.value.toLowerCase().startsWith(word)
        );
      }) == null;
    ...
    if (isTypeNonCompliantWord == true) {
      ...
      if (!parentBlock) {
        return this.buildWarningMarker(
          range,
          errors.nonCompliantResource(word)
        );
      }

      // Set the decorations on the model
      return !(suggestions[0]?.type == SuggestionType.attributeValue)
        ? this.buildWarningMarker(
            range,
            errors.nonCompliantAttr(parentBlock, word)
          )
        : this.buildWarningMarker(
            range,
            errors.nonCompliantValue(
              nestedScopes[nestedScopes.length - 1],
              word
            )
          );
    }
  ...
  validateNonCompliantWordAll(
    ...
    for (let line = 1; line <= textModel.getLineCount(); line++) {
      for (
        let column = 1;
        column <= textModel.getLineMaxColumn(line);
        column++
      ) {
        const position: any = {
          lineNumber: line,
        ...
        const skippedRange = skippedRanges.find((range) =>
          isPositionInsideRange(position, range)
        );
        ...
        if (decoration != null) {
          ...
          if (
            ...
            isOverlap(
              decoration?.range,
              decorations[decorations.length - 1]?.range
            )
          ) {
            decorations.pop();
          }
  ...
  validateNonCompliantWordLine(
    ...
    for (
      let column = 1;
      column <= textModel.getLineContent(lineNumber).length;
      column++
    ) {
      const position: any = {
      ...
      if (decoration != null) {
        ...
        if (
          decorations.length > 0 &&
          isOverlap(decoration.range, decorations[decorations.length - 1].range)
        ) {
          decorations.pop();
        }
  ...
  findSkippedRanges(textModel: monaco.editor.ITextModel): monaco.IRange[] {
    const ranges = this.hclTokenizer.getRangesByTokenType(textModel, [
      TokenType.empty,
      TokenType.string,
      TokenType.comment,
    ]);
